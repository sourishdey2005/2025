...
const Polaroid: React.FC<{ src: string; caption: string; rotation: number }> = ({ src, caption, rotation }) => (
  ...
  >
    ...
    <div className="font-cursive text-black text-center text-lg leading-none">{caption}</div>
  </motion.div>
);

const LetterCard: React.FC<{ letter: string; color: string; title: string; icon: string; polaroid?: { src: string; caption: string } }> =
    ...
    >
      ...
      <div className="relative z-10">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h3 className="font-serif text-2xl text-black font-bold uppercase tracking-widest">{title}</h3>
        </div>

        {polaroid && <Polaroid src={polaroid.src} caption={polaroid.caption} rotation={3} />}
        ...
        >
          ...
          <div className="relative z-10">
              ...
              {LETTER_CONTENT.split('\n').map((line, lineIdx) => (
                <div key={lineIdx} className={line === "" ? "h-6" : "mb-4"}>
                  {line.split(' ').map((word, wordIdx) => (
                    <motion.span key={wordIdx} variants={wordVariants} className="inline-block mr-2">
                      {word}
                    </motion.span>
                  ))}
                </div>
            ...
            <div className="mt-20 text-right md:mr-12">
              <p className="font-cursive text-2xl text-black mb-2">With all my love,</p>
              <p className="font-cursive text-4xl md:text-5xl text-black font-bold -rotate-2">Your Admirer</p>
            </div>
          </div>
        </motion.div>
        ...
        <AnimatePresence>
          {showSecret && (
            ...
            >
              ...
              >
                <Heart className="mx-auto text-black mb-4 animate-pulse relative z-10" size={48} fill="currentColor" />
                <h3 className="text-2xl font-serif text-black mb-2 relative z-10">A Secret Message</h3>
                <p className="text-black italic relative z-10">
                  "I love how your eyes light up when you're excited about something. It's my favorite sight in the world."
                </p>
                <button
                  onClick={() => setShowSecret(false)}
                  className="mt-6 px-6 py-2 bg-pink-500 text-black rounded-full font-medium transition-colors hover:bg-pink-600 relative z-10"
                >
                  Close
                </button>
              </motion.div>
